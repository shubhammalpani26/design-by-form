...
export const ORIGINALS_SKUS: OriginalSku[] = [
  {
    ...
    photo: {
      ...
      promptTemplate: (v) =>
        `Using the animal in the reference photo, create a fully three-dimensional FDM-printed PLA bust of that exact animal's head and shoulders, keeping its recognisable face, breed, ear shape, muzzle length and markings as modelled form. Sculpt the animal only — never reproduce anything it is holding or wearing in the photo: no stick, ball, toy, flower, rope, food, leash, collar, tag, harness or bandana, and no hands, background objects or other animals. Three-quarter view, rounded modelled volume front to back — absolutely not a flat silhouette, not an extruded profile plate, not a relief panel. Surface finish is one single uniform colour across the entire piece with a realistic soft satin PLA sheen — clearly photographed as a real FDM 3D print, with fine regular horizontal layer lines visibly wrapping every curved surface and the plinth, catching broad soft-box highlights; retain those lines rather than smoothing the object into stone, ceramic, resin, wax or a matte CG render. Sculpture and plinth are exactly the same solid colour, absolutely no two-tone, no colour banding, no stripes, no wood grain, no marbling, no contrasting base, no paint or metallic accents. The expression is warm and happy: a gentle open-mouthed smile with the tongue tip just visible, relaxed lifted cheeks, ears perked up and alert, and bright uplifted brows — a joyful, alive look rather than a solemn one, all rendered as sculpted form. The eyes are sculpted, not painted: deep almond sockets carved at least 2 mm into the brow with defined upper and lower lids and a smooth raised iris dome inside each socket — no glass, no inserts, no glossy beads, no paint, no thin whiskers or fine hairs. The shoulders flow directly into a compact deep rectangular plinth through a broad tapered, softly filleted transition, with no visible seam, gap, neck, pedestal or separate slab; center the bust over the plinth and slightly toward its front. Keep a straight front lettering band, generously rounded corners and soft chamfers on every outer edge — no sharp knife edges anywhere, every transition eased and hand-friendly — ${plinthLettering(v)} Monolithic and solid, one continuous part, substantial and comfortable in the hand, no thin fragile details, flat stable base, studio lighting on a clean neutral background showing depth, layer texture and shadow.`,
    },
    promptTemplate: (v) =>
