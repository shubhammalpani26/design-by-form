...
export const SHOPIFY_STOREFRONT_TOKEN = "378196950bd2f2e9e8deacb62e20fd69";
...
export async function storefrontApiRequest(query: string, variables: any = {}) {
  ...
  if (response.status === 402) {
    toast.error("Store billing required", {
      description:
        "The Shopify store needs an active billing plan before checkout can be used.",
    });
    return;
  }
...
export async function createShopifyCart(
  ...
  const data = await storefrontApiRequest(CART_CREATE_MUTATION, {
    input: {
      lines: [
        { quantity, merchandiseId: variantId, ...(attributes?.length ? { attributes } : {}) },
      ],
    },
  });
