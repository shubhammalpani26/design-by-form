...
import { actualWeightLabel } from "./originalsWeight.ts";
...
async function sendReceipt(email: string | null, orders: any[]) {
  ...
  const items = orders.map((o) => {
    ...
    return {
      ...
      skuSlug: o.sku_slug ?? "",
      quantity: qty,
      weightLabel: actualWeightLabel(o.weight_grams, o.sku_slug, o.size_key) ?? "",
    };
  });
  const templateData = {
    orderId: orders[0].id,
...
export async function markOriginalsPaid(
  ...
  const { data: orders } = await admin
    ...
    .select("id, status, sku_slug, size_key, size_label, amount_usd, quantity, preview_image_url, customer_email, weight_grams")
